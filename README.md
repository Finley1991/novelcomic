# NovelComic

小说推文 AI漫剧生成工具 & 解压视频混剪工具

## 功能特点

### 小说漫画推文（Novel Comic）
- 🎭 自动提取小说角色
- 🏞️ 自动提取小说场景
  - 基于大模型智能识别场景
  - 支持场景描述编辑
  - 分镜自动关联对应场景
- 📝 智能拆分剧本分镜
  - 支持按 1/2/3 行分割剧本
  - 原文直接填入分镜描述
  - 拆分后自动生成画图提示词
  - 分镜提示词可手动编辑
  - 分镜自动关联角色和场景
  - 增强版提示词生成：结合分镜描述+角色描述+场景描述+前后5个分镜上下文
- 🎨 AI 图片生成 (ComfyUI)
  - 支持上传和管理 ComfyUI 工作流
  - 可配置节点映射（提示词、尺寸、采样器等）
  - 每个工作流独立参数配置（宽度、高度、CFG、Steps、Seed、Sampler、Batch Size）
  - 支持正向提示词前缀/后缀
  - 支持否定提示词覆盖
- 🔊 AI 配音生成 (Microsoft TTS)
  - 24种音色可选（女声/男声/童声）
  - 支持角色独立音色配置
  - 支持分镜级独立音色配置
  - 支持批量设置音色
- ✂️ 一键导出剪映草稿 (使用 pyJianYingDraft 库，兼容剪映 5.9+)

### 解压视频混剪（Decompression Video）
- 📹 解压视频素材库管理
  - 自动扫描视频目录，获取视频时长
  - 智能视频选择算法，根据目标时长自动组合
  - 支持重新扫描视频目录
- 🎨 风格提示词库
  - 多级目录结构支持
  - 随机选择风格提示词用于图片生成
- 📝 文本分段与配音
  - 按行自动拆分文本
  - 批量生成配音（TTS）
- 🖼️ AI 图片生成
  - 根据音频时长自动计算需要的图片数量
  - 每张图片默认15秒，自动分配时间轴
  - 随机运动效果（缩放/平移）
- ✂️ 一键导出剪映草稿
  - 多轨道支持：视频（下层）+ 图片（上层）+ 音频 + 字幕
  - 默认配置：9:16竖屏、新青年体字幕、图片140%→100%缩放动画

### 通用功能
- 🤖 支持多种 LLM 提供商 (Ollama / OpenAI / 兼容 API)
- 🔌 支持 HTTP 代理配置
- 🎨 全新 UI/UX 设计
  - 温暖亲和的配色系统
  - 明暗双主题完整支持
  - 侧边栏导航 + 向导式混合布局
  - Toast 通知系统
  - 键盘快捷键支持
  - 向导式步骤导航

## 快速开始

### 前置要求

- Python 3.10+
- Node.js 18+
- ComfyUI (运行中)
- LLM 服务 (二选一):
  - Ollama (运行中)
  - OpenAI API Key (或兼容 API)
- Microsoft Azure TTS API Key (可选)

### 后端启动

```bash
cd novelcomic/backend
python -m venv venv
source venv/bin/activate  # Windows: venv\Scripts\activate
pip install -r requirements.txt
cp .env.example .env
# 编辑 .env 配置 API 地址
python main.py
```

### 前端启动

```bash
cd novelcomic/frontend
npm install
npm run dev
```

### 访问应用

- 前端: http://localhost:5173
- 后端 API: http://localhost:8000
- API 文档: http://localhost:8000/docs

## 配置说明

在运行前，请先配置以下 API 服务：

### ComfyUI

默认地址: `http://localhost:8188`

#### 工作流管理

1. **上传工作流** - 在"设置"页面上传 ComfyUI 工作流（API 格式 JSON）
2. **节点映射配置** - 配置工作流节点与字段的映射关系：
   - 正向/否定提示词节点
   - 宽度/高度节点
   - 采样器节点（包含 Steps、CFG、Seed）
   - 批次大小节点
3. **默认参数配置** - 为每个工作流配置独立的默认参数：
   - 尺寸：宽度、高度
   - 采样：Steps、CFG、Sampler、Seed
   - 批次：Batch Size
   - 提示词：正向提示词前缀/后缀、否定提示词覆盖
4. **设为激活** - 选择一个工作流作为当前激活的工作流

可以在应用的"设置"页面修改。

### LLM 设置 (二选一)

#### 选项 A: Ollama

默认地址: `http://localhost:11434`

默认模型: `qwen3.5:35b`

可以在应用的"设置"页面修改。

#### 选项 B: OpenAI / 兼容 API

支持:
- OpenAI 官方 API
- 任何兼容 OpenAI 格式的 API (如 Azure OpenAI, 本地模型等)

配置项:
- API Key: 你的 API 密钥
- Base URL: API 地址 (默认: `https://api.openai.com/v1`)
- 模型: 模型名称 (如 `gpt-4o`, `gpt-4`, `gpt-3.5-turbo`)
- 代理 (可选): HTTP 代理地址 (如 `http://127.0.0.1:7897`)

可以在应用的"设置"页面配置，并点击"测试连接"验证。

### Microsoft TTS

需要配置:
- Subscription Key
- Region

可以在应用的"设置"页面配置。

### 剪映草稿导出

剪映草稿导出功能依赖 [capcut-mate](https://github.com/Hommy-master/capcut-mate.git) 项目的 `pyJianYingDraft` 库。

**前置准备:**
1. 克隆 capcut-mate 项目到本地:
   ```bash
   git clone https://github.com/Hommy-master/capcut-mate.git
   ```
2. 在 `backend/core/jianying_exporter.py` 中配置 `CAPCUT_MATE_PATH` 指向 capcut-mate 的 src 目录

**配置说明:**
- 剪映草稿保存路径：在"设置"页面配置剪映草稿文件夹路径
- 默认画布尺寸：1920x1080 (16:9)，可在设置中修改

**使用流程:**
1. 完成图片生成和配音生成
2. 在项目编辑页面点击"导出剪映"
3. 点击"导出到剪映"按钮
4. 草稿会自动保存到配置的剪映草稿目录
5. 在剪映中打开草稿进行进一步编辑

## 使用流程

### 小说漫画推文项目

1. **配置设置** - 在"设置"页面配置 LLM、ComfyUI、TTS 等服务
   - 选择 LLM 提供商 (Ollama 或 OpenAI)
   - 填写相应的 API 配置
   - 点击"测试连接"验证配置 (特别是 OpenAI)
   - 如需使用代理，填写代理地址
2. **创建项目** - 选择"小说漫画"类型，输入项目名称，粘贴小说文本
3. **场景管理** - 自动提取或手动添加场景（可选）
   - 点击"自动生成场景"基于小说文本提取场景
   - 可编辑场景名称和描述
4. **角色管理** - 自动提取或手动添加角色（可选）
   - 点击"自动生成角色"基于小说文本提取角色
   - 可编辑角色信息、提示词和参考图片
5. **剧本拆分** - 将小说拆分为多个分镜
   - 选择每个分镜包含的行数（1/2/3行）
   - 点击"拆分剧本"自动生成分镜
   - 分镜拆分后自动关联场景和角色
   - 自动生成增强版画图提示词（结合分镜+角色+场景+前后5个分镜上下文）
   - 可在分镜列表中手动编辑提示词、场景、角色
6. **图片生成** - 批量生成漫剧画面
7. **配音生成** - 批量生成配音（支持试听）
8. **导出剪映** - 下载剪映草稿 ZIP 文件

### 解压视频混剪项目

1. **配置设置** - 在"设置"页面配置：
   - 解压视频素材目录
   - 风格提示词目录
   - ComfyUI、TTS 等服务
   - 剪映草稿保存路径
2. **创建项目** - 选择"解压视频"类型，输入项目名称，粘贴推文文本
3. **选择风格** - 从风格列表中选择图片风格
4. **拆分文本** - 按行自动拆分推文文本为片段
5. **生成配音** - 批量生成配音音频
6. **选择视频** - 自动根据总音频时长选择解压视频素材组合
7. **生成图片** - 批量生成风格图片（每张15秒）
8. **导出剪映** - 一键导出剪映草稿，包含：
   - 视频轨道：解压视频拼接
   - 图片轨道：风格图片叠加 + 缩放动画
   - 音频轨道：配音
   - 字幕轨道：文本字幕
   - 默认配置：9:16竖屏、新青年体字幕

## 项目结构

```
novelcomic/
├── backend/           # FastAPI 后端
│   ├── api/          # API 路由
│   ├── core/         # 核心业务逻辑
│   └── models/       # 数据模型
├── frontend/         # React 前端
│   ├── src/
│   │   ├── pages/    # 页面组件
│   │   └── services/ # API 客户端
└── data/             # 数据存储目录
    ├── config.json   # 全局设置
    └── projects/     # 项目数据
```

## 技术栈

**后端:**
- FastAPI
- Pydantic
- aiohttp

**前端:**
- React 18
- TypeScript
- Tailwind CSS
- Vite

**AI 服务:**
- ComfyUI (图片生成)
- LLM 提供商 (文本处理):
  - Ollama (本地/云端)
  - OpenAI / 兼容 API
- Microsoft TTS (语音合成)

## 许可证

MIT
