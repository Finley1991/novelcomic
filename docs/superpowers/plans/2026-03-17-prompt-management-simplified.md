...
**Goal:** 为 NovelComic 应用添加 Prompt 模板管理核心功能，支持自定义角色提取、分镜拆分、图像生成的提示词，提供预设模板。

**Scope:** 核心功能 - 预设模板、基本 CRUD、Settings/Project 集成。
**Out of scope:** 预览功能（后续添加）
...
在文件顶部的 Enum 定义区域（MotionType 附近）添加：
...
阅读 storage.py，确保它可以正常加载更新后的 GlobalSettings 和 Project 模型（新增的 defaultPromptTemplates 和 projectPromptTemplates 字段有默认值，应该可以正常加载）
...
找到 `generate_image_prompt` 方法，替换为：
...
找到 `extract_characters` 方法，使用与 Task 6 Step 2 完全相同的代码进行更新

- [ ] **Step 3: 修改 split_storyboard 方法**

找到 `split_storyboard` 方法，使用与 Task 6 Step 3 完全相同的代码进行更新

- [ ] **Step 4: 修改 generate_image_prompt 方法**

找到 `generate_image_prompt` 方法，使用与 Task 6 Step 4 完全相同的代码进行更新

---

## Task 8: 更新 LLMClient 统一接口
...
## Task 9: 更新 generation.py 传递项目/全局设置
...
找到 `extract_characters` 端点（大约第 178 行），更新 LLM 调用：
...
找到 `split_storyboard` 端点（大约第 200 行），更新 LLM 调用：
...
找到 `generate_single_image` 函数中调用 `generate_image_prompt` 的地方（大约第 54-62 行），更新调用：
...
## Task 11: 前端 PromptManager 页面（简化版）
