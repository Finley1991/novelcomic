...
**设计说明:**
- 实施前先调研 pyJianYingDraft 库的多轨道支持能力
- 如果支持多轨道，使用方案 A（优先）
- 如果不支持，使用方案 B（备用）
...
| 轨道 | 内容 | 说明 |
|------|------|------|
| Screen (index 0) | 解压视频 | 按时间线顺序拼接（暂不使用图片素材） |
...
5. **时间线安排（方案 A）**:
   - 视频素材: 从 0 开始按顺序完整拼接，放置在下层轨道
   - 图片素材: 从 0 开始按顺序拼接（每张 15 秒），放置在上层轨道，叠加显示
   - 音频+字幕: 精确按时间线放置（从 0 开始）
6. **时间线安排（方案 B）**:
   - 仅使用视频素材，从 0 开始按顺序完整拼接
   - 音频+字幕: 精确按时间线放置（从 0 开始）
